...
class LeaderFollowerTeleop:
    def __init__(
        ...
        *,
        ...
        leader: ServoController | None = None,
        follower: ServoController | None = None,
        close_leader: bool = True,
        close_follower: bool = True,
        relative_mode: bool = False,
        relative_gain: float = 1.0,
    ) -> None:
        self.leader = leader if leader is not None else ServoController(port=leader_port)
        self.follower = None if dry_run else (follower if follower is not None else ServoController(port=follower_port))
        self.close_leader = close_leader
        self.close_follower = close_follower
        ...
        # Absolute teleop:
        #     follower target = mapped current leader pose.
        # Relative/clutch teleop:
        #     takeover moment defines the zero offset, so the follower holds
        #     current pose until the leader moves.
        self.relative_mode = bool(relative_mode)
        self.relative_gain = float(relative_gain)
        self.relative_initialized = False
        self.relative_leader_origin_mapped: dict[int, int] = {}
        self.relative_follower_origin: dict[int, int] = {}
    ...
    def close(self) -> None:
        if self.close_follower and self.follower is not None and hasattr(self.follower, "_ser"):
            self.follower._ser.close()
        if self.close_leader and hasattr(self.leader, "_ser"):
            self.leader._ser.close()

    def step(self) -> None:
        commands = self._read_leader_positions()
        if self.relative_mode:
            self._send_relative_follower_positions(commands)
        else:
    ...
    def _init_relative_takeover(self, leader_positions: dict[int, int]) -> None:
        """Capture current leader/follower poses as the relative teleop origin."""
        follower_positions = self._read_follower_positions()

        self.relative_leader_origin_mapped.clear()
        self.relative_follower_origin.clear()

        for servo_id in self.servo_ids:
            if servo_id not in leader_positions:
                continue
            if servo_id not in follower_positions:
                continue

            leader_pos = int(leader_positions[servo_id])
            mapped_leader = self._clamp_follower_target(servo_id, leader_pos)
            follower_pos = int(follower_positions[servo_id])

            self.relative_leader_origin_mapped[servo_id] = mapped_leader
            self.relative_follower_origin[servo_id] = follower_pos
            self.last_sent[servo_id] = follower_pos

            if self.debug_targets:
                print(
                    f"[REL-INIT] servo {servo_id}: "
                    f"leader={leader_pos} mapped_leader={mapped_leader} "
                    f"follower_origin={follower_pos}"
                )

        self.relative_initialized = True

        if self.debug_targets:
            print(
                "[REL-INIT] relative teleop initialized; "
                "follower will hold until the leader moves."
            )

    def _signed_follower_delta(self, servo_id: int, origin: int, current: int) -> int:
        """Signed delta in follower coordinate, with wrap support."""
        delta = int(current) - int(origin)

        if JOINT_MAP.get(servo_id, {}).get("follower_wrap"):
            half = SERVO_TICKS_PER_TURN // 2
            delta = (delta + half) % SERVO_TICKS_PER_TURN - half

        return int(delta)

    def _send_relative_follower_positions(self, leader_positions: dict[int, int]) -> None:
        if not self.relative_initialized:
            self._init_relative_takeover(leader_positions)
            return

        for servo_id, leader_pos in leader_positions.items():
            if servo_id not in self.relative_leader_origin_mapped:
                continue
            if servo_id not in self.relative_follower_origin:
                continue

            mapped_now = self._clamp_follower_target(servo_id, int(leader_pos))
            mapped_origin = self.relative_leader_origin_mapped[servo_id]
            follower_origin = self.relative_follower_origin[servo_id]

            delta = self._signed_follower_delta(
                servo_id,
                origin=mapped_origin,
                current=mapped_now,
            )
            delta = int(round(delta * self.relative_gain))

            if JOINT_MAP.get(servo_id, {}).get("follower_wrap"):
                target = int((follower_origin + delta) % SERVO_TICKS_PER_TURN)
            else:
                target = int(follower_origin + delta)

            target = clamp_follower_target(servo_id, target)
            target = self._limit_target_step(servo_id, target)
            ...
            if last is not None and abs(target - last) < self.min_delta:
                if self.debug_targets and self.debug_unchanged:
                    print(
                        f"[REL] servo {servo_id}: leader={leader_pos} "
                        f"mapped_now={mapped_now} mapped_origin={mapped_origin} "
                        f"follower_origin={follower_origin} delta={delta} "
                        f"target={target} unchanged"
                    )
                continue
            ...
            if self.debug_targets:
                print(
                    f"[REL] servo {servo_id}: leader={leader_pos} "
                    f"mapped_now={mapped_now} mapped_origin={mapped_origin} "
                    f"follower_origin={follower_origin} delta={delta} target={target}"
                )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use relative/clutch teleop. At start, the follower holds its current pose; "
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Gain applied to leader movement deltas in --relative mode.",
    )
    parser.add_argument(
        "--print-positions",
        action="store_true",
    ...
    teleop = LeaderFollowerTeleop(
        ...
        relative_gain=args.relative_gain,
    )
