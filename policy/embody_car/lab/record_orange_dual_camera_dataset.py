...
def record_dual_grasp_episode(
    *,
    ...
    target_name = getattr(args, "marker_target", "object")
    ...
    wait_for_enter(
        f"\nPut the {target_name} in view for grasping, check wrist/external cameras, then press Enter to start recording.",
    ...
    print(f"Recording dual-camera {target_name} grasp episode: {frame_count} frames at {args.fps} fps.")
    frames_recorded = 0
    try:
...
def main() -> None:
    ...
    print(f"Recording dual-camera {args.marker_target} grasp dataset: repo_id={args.repo_id}, root={root}")
