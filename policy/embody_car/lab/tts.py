...
class TTS:
    def __init__(self, model_path: str | None = None):
        ...
        self.say_fallback = (
            sys.platform == "darwin"
            and os.environ.get("TTS_SAY_FALLBACK", "1").lower()
            not in ("0", "false", "no")
        )
        ...
        if TTS_IMPORT_ERROR is not None:
            print(f"[TTS] Kokoro 依赖不可用，改用系统语音: {TTS_IMPORT_ERROR}")
            return
        ...
        if not model_file.exists():
            print(f"[TTS] Kokoro 模型不存在，改用系统语音: {model_file}")
            return

        if not voices_file.exists():
            print(f"[TTS] Kokoro voices 文件不存在，改用系统语音: {voices_file}")
            return
        ...
        try:
            ...
            print(f"[TTS] Kokoro 初始化失败，改用系统语音: {type(exc).__name__}: {exc}")
    ...
    def _fallback_speak(self, text: str) -> None:
        ...
        if self.say_fallback:
            try:
                subprocess.run(["say", text], check=False)
                return
            except Exception as exc:
                print(f"[TTS] 系统语音播放失败: {type(exc).__name__}: {exc}")
    ...
    def speak(self, text: str, speaker: str | None = None, language: str | None = None):
        ...
        for chunk in chunks:
            try:
                ...
                print(f"[TTS] Kokoro 播放失败，改用系统语音: {type(exc).__name__}: {exc}")
                self._fallback_speak(chunk)
                return
